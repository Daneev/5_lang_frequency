...
def get_file():
    ...
    # required argument
    parser.add_argument('file', help='Enter the path to the text file',
...
def removing_punctuation_from(text):
    """
    To count the same words, you need to bring the text to
    the same register and clear of the punctuation
    """
...
def get_most_frequent_words(text, set_words_count=10):
    ...
    return list(itertools.islice(
        [(dict_freq_word[element], element) for element in sorted(
            dict_freq_word,
            key=dict_freq_word.get,
            reverse=True)
         if element != ''], set_words_count))
